...
// import 'package:protoc_plugin/const_generator.dart' show writeJsonConst;
...
class ServiceGenerator2 extends ServiceGenerator {
  ...
  ServiceGenerator2(this._descriptor, this.fileGen, Set<String> usedNames)
      : classname = disambiguateName(
            serviceBaseName(avoidInitialUnderscore(_descriptor.name)),
            usedNames,
            defaultSuffixes()),
        super(_descriptor, fileGen, usedNames);
  ...
  void _addDependency(GenerationContext ctx, String fqname, String location) {
    ...
    final mg = ctx.getFieldType(fqname) as MessageGenerator2?;
    if (mg == null) {
  ...
  void _addDepsRecursively(MessageGenerator2 mg, int depth) {
    ...
    for (final field in mg._fieldList) {
      if (field.baseType.isGroup || field.baseType.isMessage) {
        _addDepsRecursively(
            field.baseType.generator as MessageGenerator2, depth + 1);
      }
    }
  }
...
//   final ServiceDescriptorProto _descriptor;
//
//   ServiceGenerator2(super._descriptor, super.fileGen, super.usedNames)
