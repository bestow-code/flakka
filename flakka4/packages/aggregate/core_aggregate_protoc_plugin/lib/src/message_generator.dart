...
// import 'package:protoc_plugin/src/code_generator.dart';
...
class MessageGenerator2 extends ProtobufContainer {
  ...
  final List<MessageGenerator2> _messageGenerators = <MessageGenerator2>[];
  ...
  MessageGenerator2._(
    ...
    for (var i = 0; i < _descriptor.nestedType.length; i++) {
      ...
      _messageGenerators.add(MessageGenerator2.nested(
...
'Will be removed in next major version')''');
  ...
  bool _hasRequiredFields(MessageGenerator2 type, Set<String> alreadySeen) {
    checkResolved();
    ...
    for (final field in type._fieldList) {
      ...
      if (field.baseType.isMessage) {
        final child = field.baseType.generator as MessageGenerator2;
